...
**Q: パッケージ化されたアプリケーション（Shippingビルド）でも動作しますか？**

A: はい、動作します。

**Q: Windows以外のプラットフォーム（macOS, Linux, iOS, Androidなど）に対応していますか？**

A: いいえ、対応していません。

**Q: アプリケーションの実行中に新しいフォントをインストールした場合、すぐにリストに反映されますか？**

A: いいえ、反映されません。フォントリストのキャッシュはエンジン（またはエディタ）の起動時に構築されます。新しくインストールしたフォントを使用するには、エンジンまたはアプリケーションを再起動する必要があります。


**Q: このプラグインは商用利用できますか？**

A: はい、可能です。このプラグインはMITライセンスの下で提供されています。ライセンスの範囲内で自由にご利用いただけます。
